...
// embed defaults — set YOUR_BOT_ID and YOUR_TELEGRAM_USER_ID in config.json.
...
export const AppConfigSchema = z.object({
  bot_id: z.number().int().positive().default(0),
...
export function loadConfig(env: NodeJS.ProcessEnv): AppConfig {
  ...
  const stateRoot = parsedEnv.TELEGRAM_STATE_DIR
    ?? join(homedir(), '.claude', 'channels', 'tg-channel')
...
export function getStatePaths(_config: AppConfig, env: RuntimeEnv): StatePaths {
  const root = env.TELEGRAM_STATE_DIR ?? join(homedir(), '.claude', 'channels', 'tg-channel')
  return {
    root,
    env: join(root, '.env'),
