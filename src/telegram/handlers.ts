...
async function gateAndNotify(
  ...
  // Early read-receipt: set 👀 immediately when message passes the gate,
  // before any slow media build or channel notification. Fire-and-forget —
  // never blocks message delivery.
  const earlyMsgId = ctx.message?.message_id
  if (earlyMsgId !== undefined) {
    void deps.telegramApi
      .setMessageReaction(decision.chatId, earlyMsgId, '👀')
      .catch((err) => {
        deps.log.warn('early read-receipt failed (ignored)', {
          chat_id: decision.chatId,
          error: err instanceof Error ? err.message : String(err),
        })
      })
